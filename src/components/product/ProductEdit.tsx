...
const ProductEditComponent = ({
  ...
  const handleUploadProductImage = (
    ...
    if (target.files) {
      ...
      reader.onload = (e: ProgressEvent<FileReader>) => {
        ...
        editedFieldsCopy.picture = `${(e.target?.result as string).substring(
          0,
          50
        )}...`;
      ...
      switch (tagType) {
        case TagType.BUSINESS_MODELS.title: {
          ...
          if (!isRemove) productCopy.businessModels.push(newBusinessModel);
            ...
            productCopy.businessModels = productCopy.businessModels.filter(
              (b) => b.name !== value
            );
          setProductIn(productCopy);
        ...
        case TagType.CATEGORIES.title: {
          ...
          if (!isRemove) productCopy.categories.push(newCategory);
            ...
            productCopy.categories = productCopy.categories.filter(
              (c) => c.name !== value
            );
          setProductIn(productCopy);
  ...
  return (
    productIn && (
      <>
        <ModalComponent
          isActive={toggleTRLModal}
          dismiss={() => setToggleTRLModal(false)}
          title='Select TRL'
        >
          <ul>
            {TRLData?.map((trl) => (
              <li
                key={trl.id}
                className='h-14 border-b-2 hover:bg-[--secondary-color] hover:text-[--primary-color]'
              >
                <ButtonComponent
                  className='px-4 font-bold'
                  onClick={() => handleChangeTRL(trl)}
                >
                  {trl.name}
                </ButtonComponent>
              </li>
            ...
            <div className='relative bg-[--secondary-color]'>
              ...
              >
                ...
                <input
                  ...
                  id={'uploadImage'}
                  onChange={(e) =>
                    handleUploadProductImage(e, 'img_bucket_key?')
                  }
                  type='file'
                />
              </div>
              ...
              {TRLData &&
                getProductDetails(productIn).map((detail, i) => (
                  <ProductDetailComponent
                    ...
                    onChangeTags={(val: string, isRemove: boolean) => {
                      handleChangeTags(detail.title, val, isRemove);
                    }}
