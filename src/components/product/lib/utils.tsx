...
export const getProductDetails = (product: IProduct) : Array<ProductDetailType> => {
  if (product) {
    ...
    return [
      ...
      { title: Tag.BUSINESS_MODELS.title, icon: <BiBriefcaseAlt2 />, tags: businessModels},
      { title: Tag.TRL.title, icon: <BiTimer />, tags: [trl]},
      { title: Tag.INVESTMENT_EFFORT.title, icon: <BiMoney />, tags: [investmentEffort]}
    ];
  } else return [];
}
